    ...
    <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
      <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <nav className="hidden md:flex gap-6 text-sm">
          ...
          <Link href="/translate" className="hover:text-blue-600 transition">AI翻译</Link>
          <Link href="/markdown" className="hover:text-blue-600 transition">MD排版</Link>
        </nav>
      ...
      {open && (
        <nav className="md:hidden border-t bg-white px-4 py-3 flex flex-col gap-3 text-sm">
          ...
          <Link href="/translate" onClick={() => setOpen(false)} className="hover:text-blue-600">AI翻译</Link>
          <Link href="/markdown" onClick={() => setOpen(false)} className="hover:text-blue-600">MD排版</Link>
        </nav>
      )}
    </header>
  )
